...
def modulate_regression(regression_sample_quintity, regression_outlier_percentage):
    ...
    outliercount = 0
    outlier_table = np.ones(shape=regression_sample_quintity, dtype=np.int64)
...
def compare_regressions_using_lof():
    regression_model = modulate_regression(SAMPLE_SIZE, OUTLIER_PERCENTAGE)
    print("modulated with outlier count: %i" % regression_model.outliers)
    ...
    estimates_with_outliers.classify()
    estimates_with_outliers.reclassify_using_lof(outlier_table=regression_model.outlier_table)

    # estimates_with_outliers.reclassify_k_means(GroupingEstimatesDefines.RECLASSIFICATION_LEVEL)
    ...
    print("\nClasses differ with/without outliers when without reclassification: %i" % full_difference_classified)
    print("Classes differ with/without outliers when with reclassification: %i" % full_difference_reclassified)
...
def learn_on_true():
    ...
    estimates_ordinary.classify()
    rfc = estimates_ordinary.reclassify_discreminant(1.0)

    estimates_with_outliers.classify()
    estimates_with_outliers.reclassify_discreminant(1.0, rfc)
