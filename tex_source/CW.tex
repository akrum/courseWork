...
Для этого будем искать нули ее производной. Вычисление будем производить с помощью вычислительных методов (будем использовать метод секущих), так из-за сложного вида производной вычислить ее аналитически не представляется возможным.
\begin{eqnarray}
...
Если производная функции правдоподобия на точном решении будет ближе к нулю после переклассификации выборки, значит, если мы найдем ее нуль с помощью метода, описанного в пункте 5.3, то он будет ближе к истинному решению, чем если бы он находился до переклассификации выборки.
...
\begin{itemize}
    ...
    \begin{eqnarray}
        \frac{\delta l}{\delta \beta} = [ 6777.80925977~11935.60045093]^T,~\textup{при}~ \hat{\beta}= [1,1]^T,\\
        \frac{\delta l}{\delta \beta} = [ -49.05706716~283.92412386]^T,~\textup{при}~ \hat{\beta}= [90,4]^T,\\
        \frac{\delta l}{\delta \beta} = [-3129.555067~ -11908.91415502]^T,~\textup{при}~ \hat{\beta}= [170,8]^T.
    \end{eqnarray}
\end{itemize}
\section{Заключение}
