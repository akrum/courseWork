...
Для этого будем искать нули ее производной с помощью вычислительных методов(например, метода простой итерации).
\begin{eqnarray}
    ...
    =-\sum_{i=1}^{n}\begin{pmatrix}
        ...
        \dots\\
        x_{in}
    \end{pmatrix}\times  \frac{(\textup{erf'}(\frac{a_{\mu_i+1}-f(x_i,\beta)}{\sqrt{2}\sigma^2})-\textup{erf'}(\frac{a_{\mu_i}-f(x_i,\beta)}{\sqrt{2}\sigma^2}))}{ (\textup{erf}(\frac{a_{\mu_i+1}-f(x_i,\beta)}{\sqrt{2}\sigma^2})-\textup{erf}(\frac{a_{\mu_i}-f(x_i,\beta)}{\sqrt{2}\sigma^2}))}
\end{eqnarray}
