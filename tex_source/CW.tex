...
\begin{itemize}
    ...
    \item На каждой итерации будем 20 раз моделировать выборку с $N_1=1000$ и $N_2=3000$ наблюдений.
        \begin{itemize}
            ...
            \begin{eqnarray*}
                \delta_1^{\widetilde{\epsilon}_i}= \sum_{k=1}^{20}(\sum_{i=0}^{n}(\beta_i-\hat{\beta_{N_1ki}})^2)^{\frac{1}{2}}\\
                \delta_2^{\widetilde{\epsilon}_i}= \sum_{k=1}^{20}(\sum_{i=0}^{n}(\beta_i-\hat{\beta_{N_2ki}})^2)^{\frac{1}{2}}
            \end{eqnarray*}
        \end{itemize}
    ...
    \item если полученная усредненная невязка при 1000 наблюдений меньше либо равна невязке при 3000 наблюдений, то заканчиваем цикл - нашли breakdown point, т.е.:
            \begin{eqnarray}
                br=
                \begin{cases}
                    \widetilde{\epsilon_i}, if~ \widetilde{\delta_1^{\widetilde{\epsilon}_i}}<\widetilde{\delta_2^{\widetilde{\epsilon}_i}}
                \end{cases}
            \end{eqnarray}\\
...
\end{tabular}
\end{center}
Итак, видим, что М-оценки  значительно устойчивее к выбросам чем МНК. По консольному выводу можно заметить, как растет невязка у обоих методов и при этом уменьшается расхождение невязок при увеличении объема выборки, когда процент аномальных наблюдений растет.\hfill\break 
\textbf{Замечания}:
\begin{itemize}
...
Численные эксперименты показали, что МНК оценки значительно хуже применимы к аномальным наблюдениям 
по сравнению с М-оценками.
\newpage
