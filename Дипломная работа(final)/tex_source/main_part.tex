...
Для модели регрессии (\ref{eq2}), то есть для классической модели линейной регрессии, существует несколько робастных способов оценивания параметров. Далее приведены некоторые из них.
...
В ходе экспериментов (раздел \ref{sec_exp}) оказалось, что метод $K$ ближайших соседей показывает слабо исправляет ошибочные классы в выборке, поэтому было решено использовать другой подход в переклассификации.
